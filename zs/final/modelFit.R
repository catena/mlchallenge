...
set.seed(10101019)
setwd("~/workspaces/r-workspace/ideatory/zs")
...
build.gbm <- function(training) {
    ...
    grid <- data.frame(interaction.depth = 10, n.trees = 500,
...
build.c50 <- function(training) {
    ...
    grid <- data.frame(model = "rules", winnow = FALSE, trials = 20)
    cctrl <- trainControl(method = "none")
...
build.ada <- function(training) {
    ...
    grid <- data.frame(mfinal = 1000, maxdepth = 1, coeflearn = "Breiman")
    cctrl <- trainControl(method = "none")
...
runFinalModel <- function(config) {
    ...
    training <- modelData$training; testing <- modelData$testing
    modelFit <- config$build(training)
    predictions <- predict(modelFit, testing, na.action = na.pass)
    testing.orig <- readTestData()
    solution <- data.frame(Customer.ID = testing.orig$Customer.ID, 
                           Labels = 2 - as.numeric(predictions))
