...
qplot(Variable_1, data = training, fill = as.factor(Labels), geom = "density")
...
# for (x in naVars) {
#     g <- ggplot(training, aes(x = training[, x], fill = as.factor(Labels))) + 
#         geom_histogram(aes(y = ..density..), position = "dodge")
#     ggsave(filename = paste0("fig/navars/", x, ".png"), g)
# }
# r <- sapply(training[, naVars], function(x) length(unique(x)))
# M <- abs(cor(training[, c("Labels", naVars[r > 1])]))
# corrplot(M, method = "color", is.corr = F, tl.pos = "n")
...
training[, countVars] <- log10(training[, countVars] + 1)
testing[, countVars] <- log10(testing[, countVars] + 1)
...
cctrl <- trainControl(method = "cv", number = 5, 
                      ...
                      verboseIter = TRUE)
modelFit <- train(Labels ~ ., data = training, method = "glm", 
